...
The Statement has exactly three ordered subjects: `RunBundle`, complete `ConformanceReport`, and one opaque OASF record. Each uses a strict content-free `ResourceDescriptor`. The predicate records run and conformance status separately and accepts optional execution-placement evidence only as content-free descriptor bindings.

The core verifies schema, seals, artifact bindings, receipt integrity, run identity, terminal status, and issuance ordering before emission. OASF semantic validation remains external. Passport creation follows conformance evaluation and is not itself a conformance prerequisite.
...
- Raw prompts, inputs, outputs, response bodies, endpoints, errors, plans, and provider-specific payloads cannot enter through the execution-evidence contract.
